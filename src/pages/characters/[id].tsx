...
export default function CharacterId({ character }:
    ...
    const apiRequest = async (homeworldReq: any, filmsReq: any, specieReq: any) => {
        ...
        console.log(specieReq);

        if (specieReq[0]) {
            let specieRes = await fetch(specieReq[0])
                .then(response => response.json())
                .then(data => { return data })
            console.log(specieRes);
            ...
            setSpecie(
                <Link className={styles.link} href={`/species/${query.id}`}>
                    {'Human'}
                </Link>
            )
        }
                ...
                <div className={styles.container}>
                    ...
                    <div className={styles.content}>
                        ...
                        <div className={styles.info}>
                            ...
                            <p className={styles.infoLine}>Hair color: {character.hair_color}</p>
                            <p className={styles.infoLine}>Skin holor: {character.skin_color}</p>
                            <p className={styles.infoLine}>Eye holor: {character.eye_color}</p>
